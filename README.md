...
A high-performance productivity gadget for **Vectric VCarve Pro & Aspire (v12.0 / v12.5+)** that solves the major workflow hurdles when working with nested sheets:

1. **Automated Toolpath Synchronization**: Automatically loads a toolpath template (\.vtpt\) and recalculates/associates operations across **every nested sheet** in a single clickâ€”no manual sheet switching.
2. **Batch ATC NC Export**: Automatically saves separate, organized machine-ready G-code/NC files per sheet using your active ATC Post Processor (e.g. \JobName - Sheet 1.tap\, \JobName - Sheet 2.tap\).
3. **Multi-Sheet Job Setup Sheet PDF Compiler**: Compiles an interactive, unified multi-page vector PDF report for **all sheets** or **selected sheets** only, with maximized part visibility on Page 1 and clean toolpath specifications on Page 2 (zero orphan/empty pages).

---

## Features

### 1. Toolpath Synchronization Across Sheets
In standard Vectric software, applying a toolpath template only applies to the active sheet. **Nested Sheet Assist** iterates across all sheets, activates each sheet context, loads your toolpath template (\.vtpt\), and recalculates all toolpaths across all sheets automatically.

### 2. Batch ATC NC File Export
- Automatically detects toolpaths associated with each sheet.
- Saves separate machine-ready NC/TAP files per sheet directly to your designated output directory.
- Formatted naming: \<JobName> - <SheetName>.<ext>\.

### 3. High-Quality Multi-Sheet Setup Sheet PDF Generator
...
- **Optimized Layout**:
  - **Page 1**: Company header, logo, and a **maximized full-page Job Layout drawing (90vh)** for maximum part label and geometry readability.
...
- **Fast Headless PDF Conversion**: Leverages Windows built-in Microsoft Edge headless printing engine for vector-crisp PDF output.
...
3. **Configure Options**:
   - **Toolpath Template**: Select your \.vtpt\ template file (optional if toolpaths already exist).
...
Created by **Ezeor** ([@Th3-Future](https://github.com/Th3-Future)).